...
import fh_swf.mechatronik.activities.ProfileActivity;
import fh_swf.mechatronik.classes.BlueToothConnection;
import fh_swf.mechatronik.classes.LowPassFilter;
import fh_swf.mechatronik.classes.MovingCircleView;
import fh_swf.mechatronik.classes.WiFiConnection;
import fh_swf.mechatronik.fragments.CameraStreamFragment;
...
public class MainActivity extends AppCompatActivity {
    ...
    private WiFiConnection wiFiCon;
    private FrameLayout paintFrame;
    private MovingCircleView mMovingCircle;
    private Display mDisplay;
    private LowPassFilter lowpassFiltering;
    private CameraStreamFragment camera;


    /**
     * Methode zur Initialisierung des Hauptfensters beim Programmstart.
     *
     * @param savedInstanceState savedInstanceState enthält den Zustand der Aktivität.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lowpassFiltering = new LowPassFilter();
        InitializeApp();
     ...
     *
     * @param newConfig Die neue Konfiguration.
     */
    ...
    public void onResume() {
        ...
        if (optionsData != null) {
            testText1.setText(String.valueOf(optionsData.getTransmissionTime()));
            testText2.setText((optionsData.getCurrentProfile()));
        }
    }
    ...
    private void InitializeApp() {
        ...
        testText1 = (TextView) findViewById(R.id.textViewtest1);
        testText2 = (TextView) findViewById(R.id.textViewtest2);
        connectionSwitch = (Switch) findViewById(R.id.connectSwitch);
        connectionStatus = (TextView) findViewById(R.id.connectedText);
       ...
       // Sensor magnetic = mSensorManager.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD);              // Initialisierung des Zugriffs auf den Magnetfeldsensor.
        mSensorManager.registerListener(sel, accelerometer, SensorManager.SENSOR_DELAY_FASTEST);     // Registrierung des Listeners für die Auswertung des Beschleunigungssensors.
...
//        setConnectionSwitchAction();
    ...
    private void initializeAppSegwayLayout()
    {
...
//                }
                ...
                if(event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
                  //  float out[] = new float[16];
                   ...
                   // SensorManager.remapCoordinateSystem(event.values, SensorManager.AXIS_MINUS_Y, SensorManager.AXIS_MINUS_X, out);


                  //  mMovingCircle.onSensorEvent(event.values);

//                    accelXText.setText("x: "+mMovingCircle.getXcoord());
//                    accelYText.setText("y: "+mMovingCircle.getYcoord());

                    calculateSensors(lowpassFiltering.filterValues(acceleration));
     ...
     *
     * @param input Feld mit Rohwerten für die Neigung des Telefons
     */
...
//    }
     ...
     *
     * @param requestCode Interne Codenummer für die Anfrage
     * @param resultCode  Interne Codenummer für das Ergebnis
     * @param data        zu übergebende Daten für die Hauptaktivität.
     */
...
//    }
     ...
     *
     * @param id Das ausgewählte Profil.
     */
...
//        testText1.setText(optionsData.getTransmissionTime());
//        testText2.setText(optionsData.getCurrentProfile());
    }
...
//                    setContentView(R.layout.fragment_moving_circle);
//                } else {
//                    byte test = data.getButtons();
...
//    }
     ...
     *
//     * @param activeMButton Der zur Zeit aktive Button der Radio-Gruppe "RB-G1" (M1,M2 oder M3)
     * @return Ein String der die aktuellen Werte in Abhängigkeit des jeweilig aktiven Radio-Buttons anzeigt.
     */
...
//    }
        ...
        connectionSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked && optionsData.getTransmissionType() == 1) {
                    ...
                    if (btCon.getBlueSocket().isConnected()) {
                        connectionStatus.setText("BT-Connected");
                    }
                } else if (isChecked && optionsData.getTransmissionType() == 2) {
                    wiFiCon = new WiFiConnection();
                    if (wiFiCon.getWifiConnect().getUdpSocket().isConnected()) {
                ...
                if (!isChecked && optionsData.getTransmissionType() == 1) {
                    ...
                    if (wiFiCon.getWifiConnect().getUdpSocket().isConnected()) {
                        wiFiCon.getWifiConnect().getUdpSocket().disconnect();
                    }
                }
            }
        });
    ...
    private void setPaintFrameView()
    {
        //mMovingCircle = new MovingCircleView(MainActivity.this, paintFrame.getWidth(), paintFrame.getHeight());
       // mMovingCircle.setLayoutParams(new FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, FrameLayout.LayoutParams.WRAP_CONTENT));
        //paintFrame.addView(mMovingCircle);
    }
